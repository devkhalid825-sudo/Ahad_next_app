...
const VideoBg = ({
    ...
    const handleError = (event) => {
        console.warn('VideoBg load error:', event);
    };
    ...
    return (
        <div
            className={`relative w-full ${fullScreen ? 'h-full' : ''} ${
                darken ? 'after:content-[""] after:absolute after:inset-0 after:bg-gradient-to-b after:from-black/10 after:via-black/20 after:to-black/90' : ''
            } ${className}`}
        ...
        >
            {overlay && (caption || description) && (
                ...
                >
                    {caption && <h1 className="text-white text-4xl md:text-8xl font-black uppercase drop-shadow-2xl">{caption}</h1>}
                    {description && <p className="text-white max-w-3xl mx-auto mt-5 drop-shadow-lg">{description}</p>}
                </div>
            )}
            {shouldShow && (
                <video
