    ...
    if (youtubeId) {
        ...
        return (
            <div className="w-full h-full relative" style={{ overflow: 'hidden', transform: 'translateZ(0)' }}>
                ...
                <div style={{ position: 'absolute', inset: 0, zIndex: 2, background: 'transparent', cursor: 'pointer' }} />
            </div>
        );
    }
    if (instagramUrl) return <iframe src={instagramUrl} className="w-full h-full" allow="autoplay; encrypted-media" title="Instagram" loading="lazy" style={{ overflow: 'hidden', borderRadius: '24px' }} />;
    if (direct) return <video ref={videoRef} src={url} muted autoPlay loop playsInline onEnded={onEnded} className="w-full h-full object-cover bg-black rounded-[24px]" style={{ overflow: 'hidden' }} />;
    return <div className="w-full h-full flex items-center justify-center bg-black text-white/40 text-[10px] uppercase tracking-wider">Unsupported URL</div>;
};
...
const SocialIcon = ({ href, title, children, activeBg, activeBorder, activeColor }) => {
    ...
    return (
        <a href={href} target="_blank" rel="noopener noreferrer" title={title}
            ...
            onTouchStart={() => sH(true)} onTouchEnd={() => setTimeout(() => sH(false), 400)}
            onClick={handleClick}
            style={{
        ...
        >
            <span style={{ color: h ? activeColor : 'rgba(255,255,255,0.55)', transition: 'color 0.25s', display: 'flex', pointerEvents: 'none' }}>{children}</span>
        </a>
    );
};
...
const SocialMediaSection = () => {
    ...
    const toggleMute = (id, slideIdx) => {
        const isMuted = mutedStates[id] !== false;
        const ref = playerRefs.current[id];
        if (ref && ref.current) {
            try {
                const el = ref.current;
                if (el.muted !== undefined) el.muted = false;
                else if (el.contentWindow) {
                    el.contentWindow.postMessage('{"event":"command","func":"unMute","args":[]}', '*');
                }
            } catch { }
        } else {
            const slide = document.querySelector(`[data-slide-id="${slideIdx}"]`);
            if (slide) {
                const video = slide.querySelector('video, iframe');
                if (video) {
                    try {
                        if (video.muted !== undefined) video.muted = false;
                        else if (video.contentWindow) {
    ...
    const getPlayerRef = (id) => {
        if (!playerRefs.current[id]) playerRefs.current[id] = { current: null };
    ...
    const handleCardClick = useCallback((e, id, slideIdx) => {
        const ref = playerRefs.current[id];
        if (ref && ref.current) {
            try {
                const el = ref.current;
    ...
    return (
        <section className="w-full bg-black py-8 md:py-14 overflow-hidden font-sans flex flex-col justify-center relative">
            <div className="w-full relative">
                ...
                <style>{`
                    .mobile-social-pagination .swiper-pagination-bullet {
                        background: rgba(255,255,255,0.3);
                        width: 8px;
                        height: 8px;
                        opacity: 1;
                    }
                    .mobile-social-pagination .swiper-pagination-bullet-active {
                        background: #4169E1;
                        width: 24px;
                        border-radius: 4px;
                    }
                ...
                {/* Mobile Swiper - single slide with sound */}
                <Swiper
                    ref={mobileSwiperRef}
                    ...
                    {items.map((item, idx) => {
                        ...
                        return (
                            <SwiperSlide key={item.id} className="!w-full py-4">
                                <div data-slide-id={idx} className="relative w-full h-[420px] bg-[#1a1a1c] overflow-hidden"
                                    style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.7)' }}
                                    onClick={(e) => handleCardClick(e, item.id, idx)}
                                >
                                    <div className="block w-full h-full">
                                        <div className="absolute inset-0 overflow-hidden">
                                            {item.videoUrl && <VideoPlayer url={item.videoUrl} onEnded={handleVideoEnded} videoRef={getPlayerRef(item.id)} />}
                                        </div>
                                    </div>
                                ...
                                >
                                    ...
                                    {item.projectName && (
                                        <div className="absolute bottom-0 left-0 right-0 px-4 md:px-6 py-4 md:py-6 bg-gradient-to-t from-black/90 to-transparent group-hover:bg-gradient-to-t group-hover:from-[#4169E1]/80 transition-all duration-300">
                                            <span className="hidden md:block text-white text-[11px] md:text-sm font-semibold tracking-tight truncate">{item.projectName}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
