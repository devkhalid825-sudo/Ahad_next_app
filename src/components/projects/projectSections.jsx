...
export const OverviewSection = ({ overview, challenge }) => (
  <section className="px-8 py-[6rem] bg-[#F2F0EB] grid grid-cols-1 lg:grid-cols-2 gap-[3rem] items-stretch">
    <div className="p-[2rem]">
      ...
      <h2 className="text-2xl md:text-4xl lg:text-[44px] font-medium mb-10 md:mb-20 tracking-tight leading-[1.1] text-[#0D0D0D]">{overview.heading}</h2>
    ...
    <div className="bg-[#0D0D0D] rounded-lg p-[2rem] text-[#F2F0EB]">
      ...
      <h2 className="text-2xl md:text-4xl lg:text-[44px] font-medium mb-10 md:mb-20 tracking-tight leading-[1.1] text-[#F2F0EB]">{challenge.heading}</h2>
...
export const ResultsSection = ({ results, title }) => (
  <section className="bg-[#0D0D0D] px-8 py-[6rem]">
    <div className="mb-[3rem]">
      ...
      <h2 className="text-2xl md:text-4xl lg:text-[44px] font-medium mb-10 md:mb-20 tracking-tight leading-[1.1] text-[#F2F0EB]">{title}</h2>
    </div>
...
export const ProcessSection = ({ steps }) => (
  <section className="px-5 md:px-8 py-[3rem] md:py-[6rem] bg-[#F2F0EB]">
    ...
    <h2 className="text-2xl md:text-4xl lg:text-[44px] font-medium mb-8 md:mb-16 tracking-tight leading-[1.1] text-[#0D0D0D]">Our process</h2>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8">
      {steps.map((item, i) => (
        <div key={i} className="bg-white rounded-2xl p-6 md:p-8 border border-[#E0DDD7]">
          ...
          <h3 className="text-[17px] md:text-[20px] font-semibold text-[#0D0D0D] mb-3">{item.title}</h3>
    ...
    <section className="bg-[#0D0D0D] px-[15px] md:px-[40px] py-[6rem] overflow-hidden">
      ...
      {single ? (
        <div className="max-w-4xl rounded-lg overflow-hidden border border-zinc-800 bg-[#0D0D0D] aspect-[16/9] mt-[2rem]">
          <img src={images[0]} alt={`${title} showcase`} width="1280" height="720" className="w-full h-full object-cover" loading="lazy" />
        </div>
      ) : (
        <div className={`grid grid-cols-1 ${cols} gap-[15px] mt-[2rem]`}>
          {images.map((src, i) => (
            <div key={i} className="group relative overflow-hidden border border-zinc-800 hover:border-zinc-700 transition-all duration-300 aspect-[16/9] w-full rounded-lg">
              <img src={src} alt={`Project showcase ${i + 1}`} width="640" height="360" loading="lazy" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
            </div>
          ))}
        </div>
      )}
